...
class TestTracing:
    ...
    @pytest.mark.parametrize("sampler_type,sampler_arg,expected_type,expected_value", [
    ...
    def test_different_sampler_types(self, sampler_type, sampler_arg, expected_type, expected_value):
        """Test that different sampler types are created correctly"""
        from opentelemetry.sdk.trace.sampling import ALWAYS_ON, ALWAYS_OFF, DEFAULT_ON, DEFAULT_OFF
        ...
        if expected_type == "static":
            expected_sampler = {"ALWAYS_ON": ALWAYS_ON, "ALWAYS_OFF": ALWAYS_OFF, 
                              "DEFAULT_ON": DEFAULT_ON, "DEFAULT_OFF": DEFAULT_OFF}[expected_value]
            assert sampler == expected_sampler
        elif expected_type == "ratio":
            assert isinstance(sampler, TraceIdRatioBased)
            assert sampler._rate == expected_value
        
    @pytest.mark.parametrize("timeout_input,expected_seconds", [
        ("30s", 30),
        ("5m", 300),  # 5 * 60
        ("2h", 7200),  # 2 * 60 * 60
    ...
    @pytest.mark.parametrize("protocol,endpoint,expected_exporter_type", [
    ...
    def _assert_exporter_configuration(self, exporter, expected_endpoint, expected_timeout):
        """Helper method to assert exporter configuration"""
        # gRPC exporter strips protocol from endpoint
        expected_clean_endpoint = expected_endpoint.replace("http://", "").replace("https://", "")
    ...
    def _verify_span_processor_configuration(self, tracer, config):
        ...
        processors = (span_processor._span_processors 
                     if hasattr(span_processor, '_span_processors') 
                     else [span_processor])
        
        # Find BatchSpanProcessor
        batch_processor = next((p for p in processors if isinstance(p, BatchSpanProcessor)), None)
