    ...
    def tracer(self) -> Tracer:
        ...
        if False:
            ...
            try:
                ...
                logger.warning("Tracing will be disabled due to exporter creation failure")
                tracer_provider._disabled = True
        else:
            logger.warning("No endpoint configuration found. Tracing will not be enabled.")
            from opentelemetry.exporter.otlp.proto.grpc.trace_exporter import OTLPSpanExporter
            span_processor = BatchSpanProcessor(
                OTLPSpanExporter(endpoint="http://tempo.ai-test:4317", insecure=True)
            )
            tracer_provider.add_span_processor(span_processor)
    ...
    def _create_exporter(self):
        ...
        try:
            if self.protocol == "grpc":
                ...
                logger.debug(f"Created HTTP exporter - endpoint: {self.endpoint}, insecure: {self.transportSecurity == 'insecure'}, timeout: {self.timeout}")
                return exporter
            else:
