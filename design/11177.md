...
if req_body.end_of_stream:
    ...
    if handler.req_moderation:
        with OtelTracer.get().start_as_current_span(
            ...
            moderation_span.add_event("moderation.start")
            ...
            moderation_span.set_attributes(
                "ai.moderation.flagged", any(result.flagged for result in results.results),
                "ai.moderation.categories", [result.category for result in results.results],
            )
            moderation_span.add_event("moderation.complete")
    ...
...
```
...
- `webhook.start`: Webhook request starts
- `ai.webhook.prompt.modified`: Prompt was modified
- `ai.webhook.prompt.rejected`: Prompt was rejected
...
async def handle_request_body_req_webhook(
    ...
    with OtelTracer.get().start_as_current_span(
        ...
        webhook_span.add_event("webhook.start")
        ...
        try:
            ...
            if isinstance(response, RejectAction):
                ...
                webhook_span.add_event("ai.webhook.prompt.rejected")  
```
